...
def inference_detector(model, pcd):
    ...
    # forward the model
    with torch.no_grad():
        result, attn_data = model(return_loss=False, rescale=True, **data)
        data['attn_data'] = attn_data.data
    return result, data
...
def show_det_result_meshlab(data,
    ...
    attn_data = data['attn_data'].cpu().numpy()
    print('attn: ', attn_data.shape)
    ...
    if box_mode != Box3DMode.DEPTH:
        ...
        attn_data = Coord3DMode.convert(attn_data, box_mode, Coord3DMode.DEPTH)

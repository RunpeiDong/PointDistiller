...
def _write_ply(points, filename, text=True):
...
def _write_rgb_ply(points, rgbs, filename, text=True):
    points = [(points[i,0], points[i,1], points[i,2], rgbs[i,0], rgbs[i,1], rgbs[i,2]) for i in range(points.shape[0])]
...
def show_result(points,
    ...
    if attn_data is not None:
        _write_rgb_ply(attn_data[:, 0:3], attn_data[:, 3:6], osp.join(result_path, f'{filename}_attn_data.ply'))
        # _write_obj(attn_data, osp.join(result_path, f'{filename}_attn_data.obj'))

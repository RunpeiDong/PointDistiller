...
class PointRCNNRoIHead(Base3DRoIHead):
    ...
    def forward_dummy(self, x, img_metas=None, proposal_list=None):
        """Dummy forward function."""
        # features for rcnn
        backbone_feats = x['fp_features'].clone()
        backbone_xyz = x['fp_xyz'].clone()
        feats_dict = {'features': backbone_feats, 'points': backbone_xyz}

        features = feats_dict['features']
        points = feats_dict['points']
        point_cls_preds = x['points_cls_preds']
